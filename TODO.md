...
- **p-high:** Required before Popgram can reasonably replace a primary Telegram client.
- **p-mid:** Valuable follow-up after the first PoC.
- **p-low:** Deferred backlog with no near-term promise.

## First daily-drivable PoC

- [ ] **p-core:** Convert the repository to a virtual Cargo workspace with every package under `crates/`. Start with deep crates at demonstrated seams: `popgram`, `popgram-app`, `popgram-tui`, `popgram-telegram`, `popgram-store`, `popgram-media`, `popgram-config`, and the generic `compio-mtproto` and `rich-clipboard`. Keep dependencies one-way toward `popgram-app`, compose adapters only in `popgram`, and require meaningful hidden behavior before adding another crate. Run mutable application state in one asynchronous owner task; communicate with bounded typed channels, render immutable view data, and keep long-running I/O in adapter tasks.
...
- [ ] **p-core:** Accept Telegram operations optimistically while disconnected, show their Pending Action state, and retry automatically after reconnection while Popgram remains running. Preserve failed text as a Draft and expose terminal failure clearly; do not promise that pending operations survive exit or a crash.
- [ ] **p-core:** Normalize Telegram updates into Popgram-owned records and atomically commit each durable state change with its synchronization cursor before exposing it to the TUI. Persist Draft changes before reporting them saved; represent optimistic durability and network acknowledgement separately.
...
- [ ] **p-high:** Manage Scheduled Messages per Chat: schedule for an explicitly zoned local date and time, send when online where Telegram permits it, and view, edit, reschedule, delete, or send now. Keep scheduled state distinct from Drafts and Pending Actions; delivery remains server-side and survives Popgram exiting.
