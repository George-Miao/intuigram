...
- [ ] **p-core:** Rework default navigation keys around the Chat list → Composer → Active Message stack. `Left` and `Right` switch Folders from the Chat list, `Up` and `Down` move the Active Message in the Transcript, and `Esc` returns from an Active Message to the Composer. Require `Alt+Up` only when leaving a nonempty Composer for the Transcript.
- [ ] **p-core:** Use `Up` in an empty Composer to edit the most recent eligible outgoing Message. Preserve ordinary Composer cursor movement when the Draft is nonempty, and do nothing when no editable Message exists.
- [ ] **p-core:** Render and compose multiline Messages without losing line breaks. Grow and shrink the Composer from its padded minimum according to wrapped content, cap it at a useful Transcript-preserving height, and scroll its contents beyond that cap while keeping the cursor visible.
...
- [ ] **p-core:** Distinguish fresh and incremental Chat loading. Center a loading animation in an empty fresh Transcript; show `updating` beneath the Active-Chat title during partial refresh without obscuring cached Messages.
...
- [ ] **p-core:** Show sender titles and refine consecutive-message grouping, date boundaries, reply previews, media placement, spacing, and accents against Telegram Desktop and Telegram for macOS references while preserving Intuigram's dense no-bubble Transcript.
