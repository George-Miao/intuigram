use intuigram_lib::{AdapterEvent, ChatId, ScrollDirection, ScrollTarget};
use test_harness::{
...
fn opening_a_chat_loads_history_and_focuses_the_composer() -> Result<()> {
    ...
    app.screen()
        .message(40)
        .expect_delivery(intuigram_lib::DeliveryState::Read)?;
...
fn empty_composer_up_edits_the_newest_eligible_outgoing_message() -> Result<()> {
    ...
    app.screen()
        .action(intuigram_lib::Action::SaveEdit)
        .expect_available()?;
    app.expect_no_unhandled_work()
}
