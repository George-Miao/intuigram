...
use intuigram_lib::OfflineMediaFailure;

use super::*;

impl Backend {
    pub(crate) async fn execute(&mut self, effect: AdapterEffect) -> Result<Option<AdapterEvent>> {
        let AdapterEffect {
            effect,
            random_id,
            cancellation: _,
        ...
        match effect {
            ...
            Effect::ResolveOutbox { item, action } => {
                super::super::outbox::resolution::execute(&self.store, item, action).await
            }
            ...
            Effect::CacheMediaOffline { target, locator } => Ok(Some(
                match self.cache_media_offline(target, locator.as_ref()).await {
            ...
            effect @ Effect::SendPoll { .. } => self.execute_poll_send(effect, random_id).await,
            Effect::EditMessage {
                chat,
                message,
                draft_text,
            ...
            Effect::LoadMediaPreview {
                ...
                match self
                    .load_media_preview(chat, message, locator.as_ref())
                    .await
                {
            ...
            Effect::DownloadMedia {
                ...
                match self
                    .download_media(chat, message, destination, locator.as_ref())
                    .await
                {
