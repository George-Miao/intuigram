...
use crate::runtime::{EffectRoute, effect_route};
...
pub(super) async fn execute(
    ...
    let event = if matches!(effect, Effect::SetChatMediaOffline(_)) {
        ...
        match effect_route(&effect) {
            ...
            EffectRoute::TelegramControl | EffectRoute::SmallMedia | EffectRoute::LargeTransfer => {
...
) -> Result<Option<intuigram_lib::InlineImage>> {
...
mod tests {
    ...
    fn storage_and_native_io_stay_outside_the_telegram_actor() {
        ...
        assert!(handles(&Effect::SetChatMediaOffline(
            intuigram_lib::OfflineMediaPolicy {
                chat: ChatId(7),
                keep: true,
            },
        )));
    ...
    fn rich_media_file_is_read_before_actor_delivery() {
        ...
        let effect = Effect::SendRichMediaFile {
            ...
            kind: intuigram_lib::RichMediaUploadKind::File,
        ...
        assert_eq!(prepared.kind, intuigram_lib::RichMediaUploadKind::File);
    }
}
