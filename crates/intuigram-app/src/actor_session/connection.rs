...
pub(crate) struct ActorConnection {
...
impl ActorSession {
    ...
    async fn connect(
        ...
        let spawned = cluster
            ...
            .with_capacity(NonZeroUsize::new(EFFECT_CAPACITY).expect("effect capacity is non-zero"))
            .await;
        ...
        Ok(ConnectedActorSession {
            backend: Self {
                owner: Rc::new(ActorOwner {
                    ...
                    operation_providers: RefCell::new(crate::OperationProviders::production()),
                }),
            },
            events: ActorEvents::new(event_rx),
            peers: startup.peers,
            bootstrap: startup.bootstrap,
            media_limits: startup.media_limits,
        })
    }
}
