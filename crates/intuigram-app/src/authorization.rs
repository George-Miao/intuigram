pub(super) async fn resume_account(
    ...
    let account_cursor = client
        .account_synchronization_cursor()
        .await
        .context(TelegramSnafu)?;
    ...
    let current_cursors = std::iter::once(account_cursor)
        .chain(client.channel_synchronization_cursors())
        .map(store_cursor)
        .collect::<Vec<_>>();
    ...
    let media_limits = match client.media_limits().await {
        Ok(limits) => limits,
        Err(source) if !source.is_connection_failure() => MediaLimits::default(),
        Err(source) => return Err(source).context(TelegramSnafu),
    };
...
pub(super) async fn authorize_new_account(
    ...
    let cursors = std::iter::once(account_cursor)
        .chain(client.channel_synchronization_cursors())
        .map(store_cursor)
        .collect::<Vec<_>>();
    database
