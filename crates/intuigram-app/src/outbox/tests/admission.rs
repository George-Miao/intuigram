...
use crate::{
    AttachmentPayload, Clock, OperationIdSource, OperationProviders, decode_stored_message,
};
...
fn mutation_admission_does_not_rewrite_the_message_before_acknowledgement() {
    let effect = Effect::ReactMessage {
        ...
        message: Box::new(MessageView {
            ...
            delivery: intuigram_lib::DeliveryState::Sent,
            reply_to: None,
...
impl Clock for FixedClock {
    fn unix_seconds(&self) -> crate::ProviderResult<i64> {
        Ok(17)
    }
}
...
impl OperationIdSource for FixedIds {
    fn next_id(&mut self) -> crate::ProviderResult<i64> {
        Ok(41)
    }
}
