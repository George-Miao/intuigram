...
#[test]
fn image_popup_uses_larger_terminal_placement() {
    let mut current = image_message_view();
    current.media_previews = vec![intuigram_lib::MediaPreviewView {
        chat: ChatId(10),
        message: MessageId(40),
        image: intuigram_lib::InlineImage::from_rgba(64, 64, vec![255; 64 * 64 * 4])
            .expect("fixture pixels should match their dimensions"),
    }];
    current.image_popup = Some(intuigram_lib::ImagePopupView {
        chat: ChatId(10),
        message: MessageId(40),
    });
    ...
    assert!(graphics.requests()[0].size.columns > 24);
...
pub(super) fn image_message_view() -> View {
