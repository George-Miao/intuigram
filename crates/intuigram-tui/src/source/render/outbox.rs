...
pub(in crate::source) fn message_outbox<'a>(
    view: &'a View,
    message: &MessageView,
...
pub(in crate::source) fn append_message_lifecycle(
...
pub(in crate::source) fn pending_status(view: &View, width: u16) -> Option<Vec<Span<'static>>> {
    let count = view
        .outbox
        .iter()
...
pub(in crate::source) const fn is_pending(state: OutboxStateView) -> bool {
    matches!(
        state,
