...
not own a terminal. Its stateful renderer owns the optional external helper and
temporary-image lifecycle. Encoding and helper work may block, so TUI consumers
must call it outside their input and draw loop. Consumers can always use
`text_cells` while an external result is unavailable.
