...
impl App {
    pub(in crate::app) fn available_message_actions(&self) -> Vec<Action> {
    ...
    pub(in crate::app) fn open_action_menu(&mut self) {
        let (title, actions) = match self.view.focus {
            ...
            Focus::SavedDialogs => ("Dialog Actions", Vec::new()),
    ...
    pub(in crate::app) fn apply_action_menu(&mut self, action: Action) -> Option<Effect> {
        match action {
            Action::MoveUp => {
