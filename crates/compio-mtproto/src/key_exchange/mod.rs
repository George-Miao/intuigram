...
pub enum Error {
    ...
    /// Operating-system entropy was unavailable during key exchange.
    #[snafu(display("failed to obtain secure randomness for MTProto key exchange"))]
    Random { source: getrandom::Error },

    /// A configured CDN RSA public key was not valid PEM-encoded PKCS#1 or
    /// SubjectPublicKeyInfo data.
    #[snafu(display("invalid Telegram CDN RSA public key"))]
    InvalidRsaPublicKey,
...
/// Completes Telegram's authorization-key handshake against an explicit set
/// of PEM-encoded RSA trust roots.
///
/// CDN data centers use keys advertised by `help.getCdnConfig`, rather than
/// Telegram's built-in master-data-center keys.
pub async fn generate_auth_key_with_rsa_keys(
    connection: &mut BoxedTransport,
    public_keys: &[String],
) -> Result<AuthKeyMaterial> {
    let keys = public_keys
        .iter()
        .map(|key| rsa_key::RsaKey::from_pem(key).context(InvalidRsaPublicKeySnafu))
        .collect::<Result<Vec<_>>>()?;
    let mut plain = Plain::new();

    let (request, _) = authentication::step1().context(AuthenticationSnafu)?;
    let nonce = request.nonce;
    let response = invoke_plain(connection, &mut plain, &request).await?;

    let mut step2_random = [0; 32 + 224];
    getrandom::fill(&mut step2_random).context(RandomSnafu)?;
    let (request, step2) =
        custom::step2(nonce, response, &keys, &step2_random).context(AuthenticationSnafu)?;
    let response = invoke_plain(connection, &mut plain, &request).await?;

    let mut step3_random = [0; 256 + 16];
    getrandom::fill(&mut step3_random).context(RandomSnafu)?;
    let (request, step3) =
        custom::step3(step2, response, &step3_random).context(AuthenticationSnafu)?;
    let response = invoke_plain(connection, &mut plain, &request).await?;

    let finished = custom::create_key(step3, response).context(AuthenticationSnafu)?;
    Ok(AuthKeyMaterial {
        auth_key: finished.auth_key,
        time_offset: finished.time_offset,
        first_salt: finished.first_salt,
