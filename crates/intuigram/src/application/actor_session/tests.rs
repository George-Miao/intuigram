...
use compio_actor::mailbox::DeliverError;
...
fn actor_mailbox_reports_saturation_without_blocking_the_caller() {
    ...
    runtime.block_on(async {
        ...
        let (mailbox, handle) = cluster
            ...
            .capacity(NonZeroUsize::new(1).expect("one message is non-zero"))
            .await
        ...
        assert!(matches!(
            mailbox.send(2),
            Err(DeliverError::Full { message: 2 })
        ));
    ...
    runtime.block_on(async {
        ...
        let events = DeliveryEvent {
            batches: VecDeque::from([
                WorkerBatch {
                    batch: AdapterBatch {
                        event: Some(intuigram_app::AdapterEvent::OperationCompleted(
                            "first".to_owned(),
                        )),
                        peers: intuigram_telegram::PeerDirectory::default(),
                    },
        ...
        for expected in ["first", "second"] {
            ...
            assert!(matches!(
                batch.event,
                Some(intuigram_app::AdapterEvent::OperationCompleted(ref value))
                    if value == expected
            ));
        }
