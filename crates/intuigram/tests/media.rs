...
fn inline_image_padding_keeps_active_and_forwarded_rules_continuous() -> Result<()> {
    ...
    app.screen()
        .message(45)
        .expect_active_rule_ends_before_spacing()?;
    app.expect_no_unhandled_work()
}
...
#[test]
fn held_media_previews_do_not_block_navigation_and_complete_out_of_order() -> Result<()> {
    let photos = [51, 52].map(|id| {
        let mut photo = incoming(id, "Lin", format!("photo {id}"));
        photo.details.media = Some(MediaCard {
            kind: MediaKind::Photo,
            title: format!("photo-{id}.png"),
    ...
    let mut app = TestSystem::builder()
        .name("media-concurrent-navigation")
        .terminal(100, 60)
        .telegram(
            TelegramScenario::new()
                .bootstrap(
                    account("Ada")
                        ...
                        .with_history(photos.clone()),
                )
                .hold_media_preview("newest", 10, 52)
                .hold_media_preview("older", 10, 51)
    ...
    app.telegram().complete_media_preview("older")?;
    app.telegram().complete_media_preview("newest")?;
    app.press(key::UP)?;
