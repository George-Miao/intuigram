impl Client {
        ...
        let chat_views = dialogs
            ...
            .filter_map(|dialog| match dialog {
                tl::enums::Dialog::Dialog(dialog) => {
                    ...
                    let (preview, preview_sender, preview_sender_peer, preview_timestamp) =
                        top_messages
    ...
    /// Establishes the Account update checkpoint before snapshot RPCs begin.
    pub async fn account_synchronization_cursor(&mut self) -> Result<UpdateCursor> {
        let state = self
            .connection
            .invoke(&tl::functions::updates::GetState {})
            .await
            .context(InvokeSnafu)?;
        // The state subsumes anything received before this checkpoint. Updates
        // received by the following snapshot RPCs remain buffered for the live
        // stream, so no Account transition is lost.
    ...
    /// Reads the per-Channel checkpoints captured by the dialog snapshot.
    #[must_use]
    pub fn channel_synchronization_cursors(&self) -> Vec<UpdateCursor> {
        let mut channels = self.channel_pts.iter().collect::<Vec<_>>();
        channels.sort_unstable_by_key(|(chat, _)| chat.0);
        channels
            .into_iter()
            .map(|(chat, pts)| UpdateCursor {
